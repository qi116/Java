...
public class Ball {
	...
	public boolean didHitBlock(Block block){
		//side
		if((x<=block.getX()+block.getW() && x+rad*2>=block.getX()) && (y<=block.getY()+block.getL()&& y+rad*2>= block.getY()) )return true;
		return false;
	}
	...
	public void move(){
		...
		for (int i = 0; i < Coordinator.manager.blocks.size(); i++) {
			if(Coordinator.manager.blocks.get(i)!=null&&didHitBlock(Coordinator.manager.blocks.get(i))&&vy<0){
				vy = -vy;
