...
public class Tree<E extends Comparable<E>> {
	...
	public void displayGraphically(int width, int height){
		...
		// condition all nodes.
		traverseForDrawing(panel, node, node.getParent(), true, 0);
	...
	private void traverseForDrawing(DrawingPanel<E> panel, 
		...
		if(level >0){
			int x,y;
			if(isLeftMove) x = (int) (parent.getX() - panelWidth/4/Math.pow(2, level-1));
			else x = (int)(parent.getX()+ panelWidth/4/Math.pow(2, level-1));
			y = parent.getY() + verticalDistance;
			node.setXY(x, y);
