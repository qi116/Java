...
public class Ball {
	private int rad, vx, vy, x = 80, y = 150; 
	private Color color;
	private int count = 0;
	private int score = 0;
	...
	public void draw(Graphics g){
		...
		g.drawString("Score: " + score, 50, 400);
		count++;
	}
	...
	public boolean didHitBlockX(Block block){//fix this (y is taking priority unless thickness is increased
		if((x<=block.getX() && x+rad*2>=block.getX()-block.thickness) && (y<=block.getY() + block.getW()&& y+rad*2>= block.getY()) )return true;//right of block
		if((x<=block.getX()+block.getL()+block.thickness && x+rad*2>=block.getX()+block.getL()) && (y<=block.getY() + block.getW()&& y+rad*2>= block.getY()) )return true;//left of block
		return false;
	}
